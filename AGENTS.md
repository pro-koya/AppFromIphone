# Calm Brokku — プロジェクトコンテキスト

> このファイルは Codex が自動で読み込むプロジェクト指示書です。
...
現在 `app.json` の `name` は **"Calm Brokku"** だが、AdMob 登録に向け以下の3案を検討中。

| 案 | 名前 | 理由 |
|----|------|------|
| ★ 推奨 | **Calm Brokku** | 世界観と一致・ASO強・品がある |
...
### 未完了（次のタスク候補）
- [x] アプリ名の最終決定 → `app.json` 更新
- [x] iOS Bundle ID / Android Package Name の確定
- [x] AdMob アカウント登録 → 実広告への差し替え
...
- 開発ブランチ: `Codex/daily-puzzle-mvp-uuGZp`
- Push 先: `origin/Codex/daily-puzzle-mvp-uuGZp`
